...
# mycon = mysql.connector.connect(host=os.getenv('MYSQL_HOST'),
...
#         msg = Message('Your OTP', sender=os.getenv('MAIL_USERNAME'), recipients=[email])
...
#         # Fetch user details
#         user_query = """
#             SELECT user_id, date_of_birth, verification_id_no,  account_holder_name, contact_no FROM accounts WHERE user_id = %s LIMIT 1
#         """
...
#             # account_data = execute_query(query, (loan_id,), fetch='one')
#             cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
#             cur.execute("SELECT * FROM loans WHERE loan_id = %s", (loan_id,))
#             loan = cur.fetchone()

#             if loan:
#                 return render_template("View_loan_status.html", loan=loan)
#             else:
#                 flash("Loan ID not found", 'error')
                
#         except Exception as e:
#             app.logger.error(f"View loan status error: {str(e)}")
#             flash("An error occurred retrieving loan status.", 'error')
